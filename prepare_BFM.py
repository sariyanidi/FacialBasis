...
def prepare_expression_basis():
    ...
    expEV = np.loadtxt('./data/raw/std_exp.txt')

    w = np.loadtxt('./data/raw/w.dat')
    w = (w-1)
    w = w.astype(int)
    ...
    sigma_epsilons = np.sqrt(np.loadtxt('./data/raw/std_exp.txt'))/35
...
def prepare_morphable_model():
    ...
    a = scipy.io.loadmat('data/raw/01_MorphableModel.mat')
    ix_53215  = np.loadtxt('data/raw/ids_53215.dat').astype(int)
    ix_53215X = [3*int(x) for x in np.loadtxt('data/raw/ids_53215.dat')]
    ix_53215Y = [3*int(x)+1 for x in np.loadtxt('data/raw/ids_53215.dat')]
    ix_53215Z = [3*int(x)+2 for x in np.loadtxt('data/raw/ids_53215.dat')]
    ix_23660_3 = [int(x) for x in np.loadtxt('data/raw/w.dat')]
...
# landmark indices
    ...
    ext = 'mm'
    tdir = './data/BFM%s-%05d' % (ext, Nnew)
    if not os.path.exists(tdir):
        os.makedirs(tdir)
        
    tdirE = './data/BFM%s-%05d/E' % (ext, Nnew)
    ...
    ##### These offsets below are added to make results consistent with the first version of 3DI-lite! (December 2024)    
    X0new += -0.10964174951061909
    Y0new += 2.041246409895553
    Z0new += 13.164305213009339
